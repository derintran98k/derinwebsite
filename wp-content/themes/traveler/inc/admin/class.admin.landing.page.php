        ...
        public function st_optimize_metadata_post(){
            ...
            if(is_user_logged_in()) {
                ...
                if(!empty($st_optimize_meta_key) && ($st_optimize_meta_key === 'yes')){
                    $sql = "DELETE FROM `wp_postmeta` WHERE `meta_id` NOT IN (SELECT * FROM (SELECT MIN(`pm`.`meta_id`) FROM `wp_postmeta` pm GROUP BY `pm`.`post_id`, `pm`.`meta_key`) x)";
                    $wpdb->query($sql);
        ...
        static function sub_menu_list()
        {
            ...
            if (!self::register_completed()) {
                array_push($return,
                    ...
                    array(
                        'page_title' => __("Database optimization", ST_TEXTDOMAIN),
                        'menu_title' => __("Database optimization", ST_TEXTDOMAIN),
                        'menu_slug'  => 'st_admin_database_optimization'
                    )
                );
            }
