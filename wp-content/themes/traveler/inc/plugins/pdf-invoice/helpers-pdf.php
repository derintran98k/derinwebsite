...
if(!function_exists('wb_pdf_invoice_create_invoices')){
    function st_pdf_invoice_create_invoices($order_id,$order_data){
        ...
        if ($lang == 'ja' or $lang = 'zh_CN' or $lang = 'zh_TW')
            $fontFamily = 'cid0jp';
        $fontFamilySymbol = 'dejavusans';
        ...
        if($order_data->type == "normal_booking"){
            ...
            switch(get_post_type($st_booking_id)){
                ...
                case "hotel_room":
                    ...
                    $_st_room_id = get_post_meta($order_id, 'room_id', true);
                    $price_by_per_person = get_post_meta( $_st_room_id, 'price_by_per_person', true );
                    ...
                    if(!empty($discount)){
                        if ( $price_by_per_person == 'on' ) {
                            if(!empty($_st_adult_price) and !empty($_st_adult_number)){
                                ...
                                $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                                $pdf->Cell(95, 10, TravelHelper::format_money_from_db($_st_adult_price,$currency)." x ".$_st_adult_number. " x " . $diff." x ".$_st_room_num_search." = ".TravelHelper::format_money_from_db($_st_adult_price*$_st_adult_number*$diff*$_st_room_num_search,$currency), '', 0, 'R');
                                $pdf->Ln();
                            }
                            if(!empty($_st_child_price) and !empty($_st_child_number)){
                                ...
                                $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                                $pdf->Cell(95, 10, TravelHelper::format_money_from_db($_st_child_price,$currency)." x ".$_st_child_number. " x " . $diff." x ".$_st_room_num_search." = ".TravelHelper::format_money_from_db($_st_child_price*$_st_child_number*$diff*$_st_room_num_search,$currency), '', 0, 'R');
                                $pdf->Ln();
                            }
                        } else {
                            $pdf->SetFont($fontFamily, '', ($lang == 'th' ? 17 : 11));
                ...
                case 'st_activity':
                    ...
                    if(!empty($discount)){
                        ...
                        if ( isset($discount_type) && $discount_type == 'amount' ) {
                            $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                            $pdf->Cell(95, 10, TravelHelper::format_money_from_db($discount,$currency)." ", '', 0, 'R');
                        } else {
                            $pdf->SetFont($fontFamily, '', ($lang == 'th' ? 17 : 11));
                            $pdf->Cell(95, 10, $discount."% ", '', 0, 'R');
                        }
        ...
        if($order_data->type == "woocommerce"){
            ...
            switch(get_post_type($st_booking_id)){
                ...
                case "hotel_room":
                    ...
                    $_st_room_id = wc_get_order_item_meta( $order_id, '_st_room_id', true );
                    $price_by_per_person = get_post_meta( $_st_room_id, 'price_by_per_person', true );
                    $diff= $order_data->check_out_timestamp - $order_data->check_in_timestamp;
                    $diff = $diff / (60 * 60 * 24);
                    ...
                    if ( $price_by_per_person == 'on' ) {
                        if(!empty($_st_adult_price) and !empty($_st_adult_number)){
                            ...
                            $pdf->Cell(95, 10, esc_html__("Adult Price",ST_TEXTDOMAIN), '');
                            $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                            $pdf->Cell(95, 10, wc_price($_st_adult_price,array( 'currency' => $order->get_currency())). " x ". $_st_adult_number." x ".$diff." x ".$_st_room_num_search." = ".wc_price($_st_adult_price*$_st_adult_number*$diff*$_st_room_num_search,array( 'currency' => $order->get_currency())), '', 0, 'R');
                            $pdf->Ln();
                        }
                        if(!empty($_st_child_price) and !empty($_st_child_number)){
                            $pdf->SetFont($fontFamily, '', ($lang == 'th' ? 17 : 11));
                            $pdf->Cell(95, 10, esc_html__("Child Price",ST_TEXTDOMAIN), '');
                            $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                            $pdf->Cell(95, 10, wc_price($_st_child_price,array( 'currency' => $order->get_currency()))." x ". $_st_child_number." x ".$diff." x ".$_st_room_num_search." = ".wc_price($_st_child_price*$_st_child_number*$diff*$_st_room_num_search,array( 'currency' => $order->get_currency())), '', 0, 'R');
                            $pdf->Ln();
                        }
                    } else {
                        $pdf->SetFont($fontFamily, '', ($lang == 'th' ? 17 : 11));
                ...
                case 'st_activity':
                    ...
                    $discount = wc_get_order_item_meta($order_id , '_st_discount_rate' , true);
                    $discount_type = wc_get_order_item_meta($order_id , '_st_discount_type' , true);
                    if(!empty($_st_adult_price) and !empty($_st_adult_number) ){
                        $pdf->SetFont($fontFamily, '', ($lang == 'th' ? 17 : 11));
                        $pdf->Cell(95, 10, esc_html__("Adult Price",ST_TEXTDOMAIN), '');
                        $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                    ...
                    if(!empty($discount)){
                        ...
                        $pdf->SetFont($fontFamilySymbol, '', ($lang == 'th' ? 17 : 11));
                        if ( isset($discount_type) && $discount_type == 'amount' ) {
                            $pdf->Cell(95, 10, wc_price($discount, array( 'currency' => $order->get_currency())), '', 0, 'R');
                        } else {
                            $pdf->Cell(95, 10, $discount."% ", '', 0, 'R');
                        }
                        $pdf->Ln();
            ...
            case 'st_cars':
                ...
                if (!empty($order_data->check_in_timestamp)) {
                    ...
                    if (empty($time_format))
                        $time_format = 'H:i A';
                    $start_date = date_i18n($format.' '.$time_format, $order_data->check_in_timestamp);
                    $end_date = date_i18n($format.' '.$time_format, $order_data->check_out_timestamp);
