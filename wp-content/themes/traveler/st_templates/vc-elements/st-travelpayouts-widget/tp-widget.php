...
switch($widget_type){
    case 'popular-router':
        ...
        echo '<script data-optimize="0" data-no-optimize="1" async src="//www.travelpayouts.com/weedle/widget.js?width=400px&marker='.$marker.'&host='.$host.'&locale='.$language.'&currency='.$currency_default.'&destination='.$location.'" charset="UTF-8"></script>';
        break;
    case 'flights-map':
        $location = 'PAR';
        if(!empty($pr_destination['location_id'])){
    ...
    case 'hotel':
        ...
        echo '<div class="hotel-widget"><script data-optimize="0" data-no-optimize="1" charset="utf-8" async src="//www.travelpayouts.com/chansey/iframe.js?hotel_id='.$id.'&locale='.esc_attr($language).'&host='.$host.'&marker='.$marker.'.'.$add_marker.'&currency='.$currency_default.'&width=500"></script></div>';
        break;
    case 'hotel-selections':
        ...
        if($find_by == 'city'){
            ...
            echo '<script data-optimize="0" data-no-optimize="1" async src="//www.travelpayouts.com/blissey/scripts'.$language3.'.js?'.($categories).'&id='.$city_id.'&type='.$w_layout.'&currency='.$currency_default.'&width=800&host='.$host.'&marker='.$marker.'.'.$add_marker.'&limit='.$limit.'" charset="UTF-8"></script>';
        }
        ...
        if($find_by == 'hotels'){
            if(!empty($list_hotel)) {
                ...
                echo '<script data-optimize="0" data-no-optimize="1" async src="//www.travelpayouts.com/blissey/scripts' . $language3 . '.js?type=' . $w_layout . '&currency=' . $currency_default . '&width=800&host=' . $host . '&marker=' . $marker . '.' . $add_marker . '&ids='.$ids.'&limit=' . $limit . '" charset="UTF-8"></script>';
            }
        }
        break;
}